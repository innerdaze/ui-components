...
const TreeNode = ({
  ...
  // const handleTreeNodeClick = function({ event, data }) {
  // el.classList.toggle("active");
  // const _fn = el.classList.contains("active") ? onExpand : onCollapse;
  // _fn({ event, data, node: el });
  // };
  ...
  attachHandlers(
    {
      click: e => {
        if (children) {
          // handleTreeNodeClick({ event: e, data });
          onNodeClick({ event: e, data });
        } else {
          onLeafClick({ event: e, data });
        }
      },
